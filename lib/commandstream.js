...
var cmdStreamBuildGraph = function(currentNodeId, buffer, index, componentLib, graph, parent) {
    ...
    // TODO: if graph has exports, inject a top-level node, which forwards messages
    // between exported ports and the ports on internal nodes.
    // Should have node id 1, and all nodes have this as parent
    // Reuse the code in cmdStreamBuildSubGraph
    console.log(graph);
    ...
    for (var nodeName in graph.processes) {
        ...
        if (!comp) {
            console.log("Cannot find component:", process.component);
        }
